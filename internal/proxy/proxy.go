...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// only one JSON object and are left untouched.
...
// extractRealBody detects RD's double-JSON pattern and returns the second
// token when it is a success object (no "error" key) or an array.
func extractRealBody(body []byte) ([]byte, bool) {
	// Walk the first JSON object with a simple brace counter.
	...
	for i, b := range body {
		...
		case b == '}':
			...
			if depth == 0 {
				...
				// Second token is an array — forward as-is (torrent list etc.)
				if rest[0] == '[' {
					return rest, true
				}
				// Second token is an object — only forward if it's error-free.
				if rest[0] == '{' {
